    ...
    while((state = sspi_stream_->handshake()) != detail::sspi_handshake::state::done) {
      switch (state) {
        ...
        case detail::sspi_handshake::state::error_with_data:{
          // we preserve the handshake error and ignore the last write error
          boost::system::error_code ignore_ec;
          std::size_t size_written = net::write(next_layer_, sspi_stream_->handshake.out_buffer(), ignore_ec);
          if (ignore_ec) {
            return;
          }
          sspi_stream_->handshake.size_written(size_written);
